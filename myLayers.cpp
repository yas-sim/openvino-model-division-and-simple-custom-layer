...
static PyObject* conv2d(PyObject* self, PyObject* args) {
    ...
    int ndim               = PyArray_NDIM(input);                         // Number of dimensions of the input Numpy array
    ...
    int input_ch      = input_shape[1];
    int input_width   = input_shape[3];             // image height
    int input_height  = input_shape[2];             // image width
    int weight_height = weight_shape[0];
    int weight_width  = weight_shape[1];
    int weight_ch     = weight_shape[2];
    int weight_num    = weight_shape[3];

    // Create a Numpy object to store result
    PyArray_Descr* descr = PyArray_DescrFromType(NPY_FLOAT32);
    int output_width  = input_width-weight_width+1;
    int output_height = input_height-weight_height+1; 
    std::vector<npy_intp> output_shape {1, weight_num, output_width, output_height};              // Shape
    ...
    int kernel_size = weight_width;  // 3
    int stride = 1;
    int filters = weight_num;        // 64

    // Conv2D + BiasAdd + ReLU main loop
    for(int fc=0; fc<filters; fc++) {
        for(int dy=0; dy<output_height; dy+=stride) {
            for(int dx=0; dx<output_width; dx+=stride) {
                float cnv = 0.f;
                for(int cc=0; cc<input_ch; cc++) {
                    for(int fy=0; fy<weight_height; fy++) {
                        for(int fx=0; fx<weight_width; fx++) {
